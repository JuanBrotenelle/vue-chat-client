  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 401) {
        errorTitle.value = "Неверное имя пользователя или пароль";
      } else {
        errorTitle.value = error.response.data.error || "Ошибка авторизации";
      }
      ...
      errorTitle.value = "Ошибка при выполнении запроса: " + error.message;
...
<template>
  <div class="bg-white flex flex-col px-20 py-10 rounded-xl shadow-xl z-10">
    <h1 class="text-3xl font-semibold">Авторизация</h1>
    <p class="text-gray-500 mt-2">Войдите в свой аккаунт, чтобы продолжить</p>
    ...
    <div
      @keyup.enter.prevent="commonLogin()"
      class="h-10 mt-5 w-full flex items-center justify-center"
    >
      <button
        @click="commonLogin()"
        :disabled="errorTitle || !username || !password || loading"
